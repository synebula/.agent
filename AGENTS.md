...
> **最后更新**：2025-11-26 14:30  

---

## 目录概览

本项目是一个 [简要描述项目定位，1-2句话]。
...
**目录职责说明**：
- **backend/service/**：核心业务逻辑，可被多个API复用
- **backend/model/**：数据库模型定义，使用GORM标签
---

## 设计约定

### 代码风格
- **Go**：遵循 `gofmt` + `golangci-lint` 标准

### 数据库约定
- **表命名**：小写 + 下划线（`user_profiles`）
- **主键**：统一用 `id bigint`，自增
- **软删除**：使用 `deleted_at` 字段（GORM 标准）

### 依赖管理
- **Go**：使用 `go mod`，禁止直接 `vendor`
...
#### 1. 认证模块 (`backend/auth/`)
- **技术方案**：JWT Token 认证
- **中间件**：`AuthMiddleware()` 拦截所有 `/api/*` 路由

#### 2. 用户模块 (`backend/user/`)
- **核心实体**：User + UserProfile（1:1 关系）
- **关键接口**：`CreateUser()`、`GetUserById()`（缓存优先）
...
### 2025-11-15 09:00 - 项目初始化
- **变更内容**：创建项目骨架，配置开发环境
- **技术选型**：Go + React + PostgreSQL
- **关联文档**：`docs/setup.md`
